...
class WebhookResponseData(BaseModel):
    ...
    conversation_id: (
        str  # id that the source will use to determining conversation details
    )
...
class Avatar(Model):
    ...
    Attributes:
        ...
        initial_message (str): Initial message the avatar should speak upon first conversation.
    """
    ...
    slug = fields.CharField(
        255, unique=True, description="Auto-generated unique identifier"
    )
    provider = fields.CharEnumField(
        ProviderEnum, description="Provider for the avatar"
    )  # for factory method
    settings = fields.JSONField(
        description="Settings for the avatar"
    )  # these settings will be validated by the provider
    webhook = fields.ForeignKeyField(
        "persona_link.Webhook",
        related_name="avatars",
        description="Webhook to send the avatar to",
        null=True,
    )
    ...
    initial_message = fields.CharField(
        1000, null=True, description="Initial message for the avatar"
    )
    ...
    async def create_avatar(
        cls,
        data: AvatarPydantic,
        webhook: WebhookPydantic | None = None,
        initial_message: str = None,
    ):
        """
        Create an avatar with the given data
        
        Parameters:
            data (AvatarPydantic): The data for the avatar
            webhook (WebhookPydantic): The data for the webhook
            initial_message (str): The initial message for the avatar to speak
        """
    ...
    async def update_avatar(
        ...
        """
        Update the avatar with the given data
